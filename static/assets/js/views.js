...
async function renderPOS(c) {
  ...
  c.innerHTML = `
    <div class="pos-container" style="display:grid; grid-template-columns:1fr 360px; gap:20px; min-height:calc(100vh - 160px);">
      <div class="pos-products">
        <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:16px; gap:14px; flex-wrap:wrap;">
          <div class="search-wrap" style="position:relative; flex:1; max-width:300px;">
            <i class="fas fa-search" style="position:absolute; left:14px; top:50%; transform:translateY(-50%); color:var(--text-muted);"></i>
            <input type="text" id="pos-search" class="input-field" placeholder="Search menu products..." style="padding-left:38px; width:100%;" oninput="filterPOSGrid()">
          </div>
          <div class="pos-categories" id="pos-cat-bar" style="display:flex; gap:8px; overflow-x:auto;">
            <button class="btn btn-primary btn-sm" onclick="selectPOSCategory('all', this)">All Items</button>
            ${categories.map(cat => `<button class="btn btn-outline btn-sm" onclick="selectPOSCategory(${cat.id}, this)">${cat.name}</button>`).join('')}
          </div>
        </div>

        <div class="pos-grid" id="pos-grid" style="display:grid; grid-template-columns:repeat(auto-fill, minmax(170px, 1fr)); gap:14px;">
          ${renderPOSProductCards(products)}
        </div>
      </div>

      <div class="pos-cart section-card" style="display:flex; flex-direction:column; padding:0; height:fit-content; position:sticky; top:90px;">
        <div class="section-head" style="padding:14px 18px;">
          <h2 style="font-size:15px;"><i class="fas fa-shopping-bag" style="color:#c9a227; margin-right:6px;"></i> Current Cart</h2>
          <button class="btn btn-outline btn-sm" onclick="clearCart()" style="font-size:11px; padding:4px 10px;">Clear</button>
        </div>

        <div class="cart-items" id="cart-items-container" style="flex:1; max-height:360px; overflow-y:auto; padding:14px 18px;">
          ${renderCartItems()}
        </div>

        <div class="cart-footer" style="padding:16px 18px; border-top:1px solid rgba(205,190,150,0.25); background:rgba(0,0,0,0.01);">
          ...
          <div style="display:flex; justify-content:space-between; margin-bottom:14px; font-size:16px; font-weight:800; color:var(--text-main);">
            <span>Total Payable:</span>
            <span id="cart-total" style="color:#c9a227;">₱0.00</span>
          </div>
          <button class="btn btn-primary" style="width:100%; height:44px; font-size:14px; font-weight:700;" onclick="openPaymentModal()">
            <i class="fas fa-money-bill-wave"></i> Charge & Checkout
          </button>
        </div>
      </div>
    </div>
...
function renderPOSProductCards(products) {
  if (!products || !products.length) {
    return `<div style="grid-column:1/-1; text-align:center; padding:36px; color:var(--text-muted);"><i class="fas fa-utensils" style="font-size:32px; margin-bottom:10px; opacity:0.3;"></i><br>No menu items found.</div>`;
  }
  return products.map(p => `
    <div class="pos-item-card stat-card" style="padding:16px; flex-direction:column; align-items:flex-start; gap:8px; cursor:pointer; position:relative; overflow:hidden;" onclick="addToCart(${p.id}, '${p.name.replace(/'/g, "\\'")}', ${p.price})">
      <div style="display:flex; justify-content:space-between; width:100%; align-items:flex-start;">
        <span class="badge badge-info" style="font-size:10px; padding:2px 8px;">${p.category ? p.category.name : 'Food'}</span>
        <span style="width:28px; height:28px; border-radius:50%; background:rgba(201,162,39,0.12); color:#c9a227; display:flex; align-items:center; justify-content:center; font-size:12px;"><i class="fas fa-plus"></i></span>
      </div>
      <div style="font-weight:700; font-size:14px; color:var(--text-main); line-height:1.3; min-height:36px;">${p.name}</div>
      <div style="margin-top:auto; display:flex; justify-content:space-between; align-items:baseline; width:100%;">
...
function renderCartItems() {
  if (!posCart.length) {
    return `<div style="text-align:center; padding:36px 12px; color:var(--text-muted); font-size:13px;"><i class="fas fa-shopping-bag" style="font-size:28px; margin-bottom:10px; opacity:0.3;"></i><br>Cart is currently empty.<br><span style="font-size:11px;">Click on any menu item to add.</span></div>`;
  }
  return posCart.map((i, idx) => `
    <div style="display:flex; justify-content:space-between; align-items:center; padding:10px 0; border-bottom:1px solid rgba(205,190,150,0.15); font-size:13px;">
      <div style="flex:1; padding-right:8px;">
        <div style="font-weight:600; color:var(--text-main); font-size:13px; line-height:1.2;">${i.name}</div>
        <div style="color:#c9a227; font-weight:700; font-size:12px; margin-top:2px;">${Utils.formatCurrency(i.price * i.quantity)}</div>
      </div>
      <div style="display:flex; align-items:center; gap:6px;">
        <button type="button" class="btn btn-outline btn-sm" style="padding:2px 8px; font-size:11px;" onclick="updateCartItemQty(${i.product_id}, -1)">-</button>
        <span style="font-weight:700; min-width:18px; text-align:center; font-size:13px;">${i.quantity}</span>
        <button type="button" class="btn btn-outline btn-sm" style="padding:2px 8px; font-size:11px;" onclick="updateCartItemQty(${i.product_id}, 1)">+</button>
        <button type="button" class="btn-icon" onclick="posCart.splice(${idx}, 1); updateCartUI();" style="border:none; background:transparent; color:var(--danger); cursor:pointer; font-size:13px; margin-left:4px;" title="Remove"><i class="fas fa-trash-alt"></i></button>
      </div>
    </div>
  `).join('');
}
  ...
  Utils.openModal("Process Order Checkout", `
    <form id="form-payment" onsubmit="event.preventDefault(); submitCheckout();">
      ...
      <div class="input-group" style="margin-bottom:14px;">
        ...
        <input type="number" step="0.01" id="pay-tendered" class="input-field" required value="${total}" style="width:100%; font-size:16px; font-weight:700;" oninput="calculateChange(${total})">
      </div>
...
async function renderSuppliers(c) {
  ...
  try {
    const [statsRes, suppRes] = await Promise.all([
      Auth.fetch("/api/v1/suppliers/stats"),
      Auth.fetch("/api/v1/suppliers")
    ]);

    if (!statsRes.ok || !suppRes.ok) throw new Error("Failed to load suppliers data.");

    const statsData = await statsRes.json();
    const suppliersData = await suppRes.json();
    globalSuppliers = Array.isArray(suppliersData) ? suppliersData : [];

    const cats = Object.entries(statsData.categories || {})
      .sort((a, b) => b[1] - a[1])
      .slice(0, 3);
    const icons = ["green", "orange", "purple"];
    const faIcons = ["fa-check-circle", "fa-leaf", "fa-box-open"];

    const categoryCardsHtml = cats.map(([name, count], i) => `
      <div class="stat-card">
        <div class="stat-icon ${icons[i]}"><i class="fas ${faIcons[i]}"></i></div>
        <div class="stat-info">
          <h3>${name}</h3>
...
async function submitAddSupplier() {
  ...
  try {
    ...
    console.error("[SUPPLIERS] Submit error:", error);
