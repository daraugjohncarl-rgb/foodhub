document.addEventListener("DOMContentLoaded", () => {
  ...
  buildSidebarMenu("admin", navigate);
...
function updateNotifBadge() {
  const unread = DB.notifications.filter(n => !n.read && (n.targetRole === "admin" || !n.targetRole)).length;
  document.getElementById("notif-count").innerText = unread;
}

function showNotifications() {
  const myNotifs = DB.notifications.filter(n => n.targetRole === "admin" || !n.targetRole);
...
function markNotifsRead() {
  DB.notifications.forEach(n => {
    if (n.targetRole === "admin" || !n.targetRole) n.read = true;
  });
  saveDB();
  updateNotifBadge();
  closeModal();
