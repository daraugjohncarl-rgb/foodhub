document.addEventListener("DOMContentLoaded", () => {
  ...
  buildSidebarMenu("cashier", navigate);
  
  // Default view for cashier is POS
  navigate("pos", "POS / Sales");
...
function navigate(viewId, title) {
  if (!hasPermission("cashier", viewId)) {
...
function updateNotifBadge() {
  const unread = DB.notifications.filter(n => !n.read && (n.targetRole === "cashier" || !n.targetRole)).length;
  document.getElementById("notif-count").innerText = unread;
}

function showNotifications() {
  const myNotifs = DB.notifications.filter(n => n.targetRole === "cashier" || !n.targetRole);
...
function markNotifsRead() {
  DB.notifications.forEach(n => {
    if (n.targetRole === "cashier" || !n.targetRole) n.read = true;
  });
  saveDB();
  updateNotifBadge();
  closeModal();
...
function handleStorageSync(e) {
  ...
  // Cashier cross-tab notification
  const currentView = document.getElementById("current-page-title").innerText.toLowerCase();
  const ready = DB.kitchenOrders.filter(o => o.status === "COMPLETED");
  if (currentView.includes("pos") && ready.length) {
    // Optionally notify without spamming
  }
