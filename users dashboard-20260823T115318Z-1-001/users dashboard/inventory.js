document.addEventListener("DOMContentLoaded", () => {
  ...
  buildSidebarMenu("inventory", navigate);
...
function navigate(viewId, title) {
  if (!hasPermission("inventory", viewId)) {
...
function updateNotifBadge() {
  const unread = DB.notifications.filter(n => !n.read && (n.targetRole === "inventory" || !n.targetRole)).length;
  document.getElementById("notif-count").innerText = unread;
}

function showNotifications() {
  const myNotifs = DB.notifications.filter(n => n.targetRole === "inventory" || !n.targetRole);
...
function markNotifsRead() {
  DB.notifications.forEach(n => {
    if (n.targetRole === "inventory" || !n.targetRole) n.read = true;
  });
  saveDB();
  updateNotifBadge();
  closeModal();
